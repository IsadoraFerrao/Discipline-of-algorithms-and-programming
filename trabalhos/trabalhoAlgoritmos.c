...
int main() {
   float A, B, erro, intervalo, ponto_medio, iteracoes; // A e B : pontos | Intervalo: tamanho| iteracoes: número de iterações
   int teorema = 0; // Teorema do excelentissímo sr. Bolzano
    ...
    scanf("%f %f", &A, &B);
    teorema=(f(A)*f(B)>0); //Após testar ocorre uma troca de sinal no intervalo, existindo assim uma raíz.

   while(teorema == 0) {
      printf("\n Querido, seu intervalo [%f : %f] não condiz com o Teorema. Mas você tem uma nova chance, tente novamente \n : ",A,B);
      scanf("%f %f", &A, &B);
      teorema= (f(A)*f(B) > 0);
                        }
